...
export default function useChat(): ChatHook {
    ...
    try {
      ...
      await supabase
        ...
        .update({ 
          ...
          ...(messages.length === 0 && message.role === 'user' ? { title: message.content.substring(0, 50) } : {})
        })
  ...
  const generateImage = useCallback(async (prompt: string) => {
    try {
      ...
      const savedUserMessage = user ? await saveMessage(userMessage) : userMessage;
      ...
      const savedAssistantMessage = user ? await saveMessage(assistantMessage) : assistantMessage;
      ...
      const savedErrorMessage = user ? await saveMessage(errorMessage) : errorMessage;
  ...
  const sendMessage = useCallback(async (text: string, characterId?: string | number) => {
    ...
    try {
      ...
      // Save assistant message if authenticated
      const savedBotMessage = user ? await saveMessage(botMessage) : botMessage;
  ...
  }, [messages, user, currentConversationId, startNewConversation]);
